@extends('site.layouts.main')
@section('title', __('Repost page'))


@section('content')
    <section>
        <div class="bg-img">
            <img src="{{Vite::asset('resources/images/icons/background/78.jpg')}}" alt="">
            <div class="bg-layer"></div>
            <div class="container px-3 flex justify-center items-center h-full">
                <p class="size8 w-full  text-center text-white">
                    {{ $currentPage['title_'.app()->getLocale()] }}
{{--                    TUKRISTON INVEST AKSIONERLIK JAMIYATINING 2021-yil uchun yillik hisoboti--}}
                </p>
            </div>
        </div>

        <div class="container flex justify-between px-3 py-10">

            <div class="menu border-r pr-5">
                <div class="logo mb-8"></div>

                <p class="size3 font-medium text-center my-5">HISOBOT DAVRINI TANLANG</p>
                <form action="{{ route('repost-filter') }}" method="POST">
                    <div class="flex items-center gap-2">
                        <select>
                            <option value="2022-01-01 00:00:00">2022</option>
                            <option value="2021-01-01 00:00:00">2021</option>
                            <option value="2020-01-01 00:00:00">2020</option>
                        </select>
                        <button class="button py-2">TANLASH</button>
                    </div>
                </form>

                @foreach($allPages as $page)
                    <div class="my-5">
                        <p class="size3">{!! $page['title_'.app()->getLocale()] !!}</p>
                        <a href="{{'/repost/'.$page->slug}}" class="size3 text-link">{{ substr($page->created_at,0,10) }}</a>
                    </div>
                @endforeach
{{--                <div class="my-5">--}}
{{--                    <p class="size3">Lorem ipsum dolor sit amet consectetur. Ac varius nisl erat ut eget viverra faucibus feugiat id.</p>--}}
{{--                    <a href="#" class="size3 text-link">1 - yanvar 2021 yil</a>--}}
{{--                </div>--}}

{{--                <div class="my-5">--}}
{{--                    <p class="size3">Lorem ipsum dolor sit amet consectetur. Ac varius nisl erat ut eget viverra faucibus feugiat id.</p>--}}
{{--                    <a href="#" class="size3 text-link">1 - yanvar 2021 yil</a>--}}
{{--                </div>--}}

{{--                <div class="my-5">--}}
{{--                    <p class="size3">Lorem ipsum dolor sit amet consectetur. Ac varius nisl erat ut eget viverra faucibus feugiat id.</p>--}}
{{--                    <a href="#" class="size3 text-link">1 - yanvar 2021 yil</a>--}}
{{--                </div>--}}
            </div>

            <div class="content pl-5">
                <div class="size5 mb-2">
                    {{ $currentPage['title_'.app()->getLocale()] }}
{{--                    TUKRISTON INVEST AKSIONERLIK JAMIYATINING 2021-yil yillik hisoboti--}}
                </div>

                <div class="flex justify-between w-fit">
                    <div class="flex">
                        <span class="icon-news-user flex items-center"></span>
                        <p class="size2 text-mute ml-1">{{ strtoupper($currentPage->createdBy->name[0]) }}Admin Admin</p>
                    </div>
                    <div class="flex mx-3">
                        <span class="icon-news-calendar flex items-center"></span>
                        <p class="size2 text-mute ml-1">{{ substr($currentPage->created_at, 0, 10) }}01-01-2021</p>
                    </div>
                    <div class="flex">
                        <span class="icon-news-eye flex items-center"></span>
                        <p class="size2 text-mute ml-1">{{ $currentPage->view }}1200</p>
                    </div>
                </div>

                <p class="size4 mt-10">
                    {!! $currentPage['content_'.app()->getLocale()] !!}
{{--                    Lorem ipsum dolor sit amet consectetur. Tempus id sed viverra nibh lorem. Pellentesque nulla ullamcorper quam lorem nulla amet vel etiam proin. Scelerisque senectus nullam enim viverra neque. Eget mattis orci lorem vitae faucibus sit ut nulla. Ultrices eu elementum interdum in elit porta lectus in euismod. Cras sed vitae tortor nisi. Mauris mauris sollicitudin accumsan pellentesque egestas lorem facilisi nibh. Vulputate lorem ultrices magna dictum faucibus et consequat. Et neque metus congue at scelerisque imperdiet laoreet. Habitant auctor cras eget sed morbi ut convallis tempus risus. Est lobortis sed elit hendrerit mi diam interdum posuere id. Cursus pellentesque in pharetra diam vitae. Sed neque elementum dignissim tortor sem eget risus.--}}
{{--                    Dolor faucibus eget porta ullamcorper ut bibendum non leo. Nunc at ac nam feugiat ornare. Pharetra rutrum at mi at proin faucibus eu. Sit feugiat at lectus adipiscing. Neque ipsum arcu sed eget turpis facilisi facilisis cras nunc. In amet pellentesque nunc pharetra tristique. Sit viverra nisl mattis viverra. Erat neque lacus in bibendum enim. Ullamcorper adipiscing risus adipiscing pulvinar urna turpis placerat lobortis. At vitae enim at ornare gravida tincidunt urna. Adipiscing porta facilisi id id sed amet ac.--}}
{{--                    Tortor consectetur integer enim molestie velit sit sapien id. Turpis pellentesque eget gravida netus ullamcorper lectus etiam amet eu. Duis neque morbi odio tincidunt vestibulum sed ut amet. Scelerisque amet porttitor sit habitasse amet tellus mattis dictum. Odio aliquam ultricies adipiscing mauris purus. Morbi tortor massa diam tellus nulla tristique commodo euismod. Semper enim egestas amet in. Vulputate convallis in placerat amet consectetur odio aenean at. Nisl aliquet congue ultrices commodo. Facilisi eget posuere ut fermentum cursus suspendisse amet. Urna lectus neque aliquam vitae elementum quis sed pellentesque. Dui id nibh non viverra.--}}
{{--                    Adipiscing habitasse risus velit sit ac pretium hendrerit. Aliquam quam id habitant faucibus hendrerit faucibus. Odio mattis amet ultrices augue elementum aliquam et ipsum sed. Est venenatis lorem urna porttitor quisque urna. Parturient mi lacus mauris sem nunc a. Euismod auctor magna diam mi donec iaculis. Ultrices faucibus risus quisque est. Vel metus arcu molestie condimentum urna. Egestas ultrices vestibulum pellentesque phasellus at a turpis et quisque. Et vitae aliquet orci libero id amet donec erat ipsum. A nibh et ornare purus lacinia eu.--}}
{{--                    Venenatis montes lacus est egestas. Neque hendrerit interdum eget a enim consectetur. Vel congue enim dictum nunc ut nulla auctor facilisis. Massa ipsum rutrum mauris dapibus. Odio tortor morbi dolor scelerisque eu nam mattis nascetur. Enim praesent turpis enim sem condimentum molestie arcu adipiscing. Natoque arcu viverra in nisi tempor.--}}
{{--                    Aliquet quis iaculis massa iaculis felis. Eget nisl volutpat nisl ornare mattis nulla. Ultrices lacus in velit viverra tristique lobortis est in tempor. Nec tortor vel mauris imperdiet iaculis tempor. Eu a sit iaculis elementum leo odio aliquam. Pretium vitae et bibendum mattis. Condimentum ultrices consectetur adipiscing gravida. Senectus lectus ut nunc mauris odio. Urna morbi enim gravida semper risus arcu convallis neque. Tincidunt fringilla et magna at habitant. Magnis et sed quisque ac malesuada amet. Egestas elit vitae et aliquam cursus. Habitasse condimentum pulvinar leo nulla massa bibendum fusce nibh mi. Eget eget fames enim lorem.--}}
{{--                    Enim ac sit mauris orci vitae semper id quis sagittis. Leo consectetur quis facilisis cursus. Rutrum risus neque viverra praesent molestie pulvinar. Maecenas mattis velit justo molestie. Bibendum ullamcorper morbi ut id. Velit semper tellus nulla nam dignissim dolor at non. Velit metus risus ac integer urna magna auctor scelerisque urna. At ut feugiat volutpat ultrices ac purus donec. Vitae eu convallis tortor rutrum imperdiet non facilisis. Ullamcorper lectus arcu tortor in elit arcu mollis. Sed velit purus eu nullam lorem eu tincidunt nibh venenatis. Consectetur vitae sodales egestas dictumst.--}}
{{--                    Sem turpis ipsum pharetra enim mollis interdum malesuada purus. Et eget velit nunc duis sed arcu eu ipsum elementum. Quis odio lacus imperdiet mi aenean vivamus pretium. Nibh integer nunc commodo aliquet sit. Orci in a tortor enim nam ut viverra. Eget morbi risus ac sit vel malesuada malesuada et leo.--}}
{{--                    Lorem ipsum dolor sit amet consectetur. Tempus id sed viverra nibh lorem. Pellentesque nulla ullamcorper quam lorem nulla amet vel etiam proin. Scelerisque senectus nullam enim viverra neque. Eget mattis orci lorem vitae faucibus sit ut nulla. Ultrices eu elementum interdum in elit porta lectus in euismod. Cras sed vitae tortor nisi. Mauris mauris sollicitudin accumsan pellentesque egestas lorem facilisi nibh. Vulputate lorem ultrices magna dictum faucibus et consequat. Et neque metus congue at scelerisque imperdiet laoreet. Habitant auctor cras eget sed morbi ut convallis tempus risus. Est lobortis sed elit hendrerit mi diam interdum posuere id. Cursus pellentesque in pharetra diam vitae. Sed neque elementum dignissim tortor sem eget risus.--}}
{{--                    Dolor faucibus eget porta ullamcorper ut bibendum non leo. Nunc at ac nam feugiat ornare. Pharetra rutrum at mi at proin faucibus eu. Sit feugiat at lectus adipiscing. Neque ipsum arcu sed eget turpis facilisi facilisis cras nunc. In amet pellentesque nunc pharetra tristique. Sit viverra nisl mattis viverra. Erat neque lacus in bibendum enim. Ullamcorper adipiscing risus adipiscing pulvinar urna turpis placerat lobortis. At vitae enim at ornare gravida tincidunt urna. Adipiscing porta facilisi id id sed amet ac.--}}
{{--                    Tortor consectetur integer enim molestie velit sit sapien id. Turpis pellentesque eget gravida netus ullamcorper lectus etiam amet eu. Duis neque morbi odio tincidunt vestibulum sed ut amet. Scelerisque amet porttitor sit habitasse amet tellus mattis dictum. Odio aliquam ultricies adipiscing mauris purus. Morbi tortor massa diam tellus nulla tristique commodo euismod. Semper enim egestas amet in. Vulputate convallis in placerat amet consectetur odio aenean at. Nisl aliquet congue ultrices commodo. Facilisi eget posuere ut fermentum cursus suspendisse amet. Urna lectus neque aliquam vitae elementum quis sed pellentesque. Dui id nibh non viverra.--}}
{{--                    Adipiscing habitasse risus velit sit ac pretium hendrerit. Aliquam quam id habitant faucibus hendrerit faucibus. Odio mattis amet ultrices augue elementum aliquam et ipsum sed. Est venenatis lorem urna porttitor quisque urna. Parturient mi lacus mauris sem nunc a. Euismod auctor magna diam mi donec iaculis. Ultrices faucibus risus quisque est. Vel metus arcu molestie condimentum urna. Egestas ultrices vestibulum pellentesque phasellus at a turpis et quisque. Et vitae aliquet orci libero id amet donec erat ipsum. A nibh et ornare purus lacinia eu.--}}
{{--                    Venenatis montes lacus est egestas. Neque hendrerit interdum eget a enim consectetur. Vel congue enim dictum nunc ut nulla auctor facilisis. Massa ipsum rutrum mauris dapibus. Odio tortor morbi dolor scelerisque eu nam mattis nascetur. Enim praesent turpis enim sem condimentum molestie arcu adipiscing. Natoque arcu viverra in nisi tempor.--}}
{{--                    Aliquet quis iaculis massa iaculis felis. Eget nisl volutpat nisl ornare mattis nulla. Ultrices lacus in velit viverra tristique lobortis est in tempor. Nec tortor vel mauris imperdiet iaculis tempor. Eu a sit iaculis elementum leo odio aliquam. Pretium vitae et bibendum mattis. Condimentum ultrices consectetur adipiscing gravida. Senectus lectus ut nunc mauris odio. Urna morbi enim gravida semper risus arcu convallis neque. Tincidunt fringilla et magna at habitant. Magnis et sed quisque ac malesuada amet. Egestas elit vitae et aliquam cursus. Habitasse condimentum pulvinar leo nulla massa bibendum fusce nibh mi. Eget eget fames enim lorem.--}}
{{--                    Enim ac sit mauris orci vitae semper id quis sagittis. Leo consectetur quis facilisis cursus. Rutrum risus neque viverra praesent molestie pulvinar. Maecenas mattis velit justo molestie. Bibendum ullamcorper morbi ut id. Velit semper tellus nulla nam dignissim dolor at non. Velit metus risus ac integer urna magna auctor scelerisque urna. At ut feugiat volutpat ultrices ac purus donec. Vitae eu convallis tortor rutrum imperdiet non facilisis. Ullamcorper lectus arcu tortor in elit arcu mollis. Sed velit purus eu nullam lorem eu tincidunt nibh venenatis. Consectetur vitae sodales egestas dictumst.--}}
{{--                    Sem turpis ipsum pharetra enim mollis interdum malesuada purus. Et eget velit nunc duis sed arcu eu ipsum elementum. Quis odio lacus imperdiet mi aenean vivamus pretium. Nibh integer nunc commodo aliquet sit. Orci in a tortor enim nam ut viverra. Eget morbi risus ac sit vel malesuada malesuada et leo.--}}
                </p>

            </div>

        </div>
    </section>
@endsection
